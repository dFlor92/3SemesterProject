    ...
    public class DBSession : IDatabaseCRUD<Session>
    {
        ...
        public void Create(Session entity)
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                using (SqlCommand cmd = conn.CreateCommand())
                {
                    cmd.CommandText = "INSERT INTO Session (start, end, systemUserId, leadId) VALUES (@start, @end, @systemUserId, @leadId)";
                    cmd.Parameters.AddWithValue("@date", entity.Start);
        ...
        public void Update(Session entity)
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                using (SqlCommand cmd = conn.CreateCommand())
                {
                    cmd.CommandText = "UPDATE Session SET(start = @start, end = @end, systemUserId = @systemUserId, leadId = @leadId) WHERE id = @id";
